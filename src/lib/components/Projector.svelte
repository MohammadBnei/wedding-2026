<!--
  The projector: the guest wall as it ran on the night, one slide at a time.

  Lifted out of routes/wall/+page.svelte so that route can switch between this
  and Gallery.svelte without this component's timers, polls and listeners
  running behind the gallery — they start on mount, so an `{#if}` around the
  markup alone would not have stopped them.

  Everything below is as it ran, with one deliberate exception: see the
  `:global(body:has(.wall))` note in the style block.
-->
<script>
...
<style>
  /*
    Scoped to the projector's own root on purpose, and this is load-bearing.

    `{#if}` in the route gates INSTANTIATION, not CSS: Svelte emits `:global()`
    unscoped and SvelteKit ships the stylesheet of every component statically
    imported by the route, rendered or not. A bare `:global(body)` here would
    therefore lock scrolling on the post-wedding gallery too — which is a
    scrolling grid, so it would silently stop working with nothing to see in
    either file. `.wall` exists only while this component is mounted, so
    `:has()` ties the lock to the thing that actually needs it.

    The `background` that used to sit here is gone rather than guarded:
    app.css already sets it on body for every page, and being the shorthand it
    also reset the girih tile app.css puts on `[data-theme='dark'] body`.
    Invisible under a `position: fixed; inset: 0` stage, but not something the
    gallery should inherit.
  */
  :global(body:has(.wall)) {
    overflow: hidden;
  }
